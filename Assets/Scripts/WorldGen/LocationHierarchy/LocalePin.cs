...
public class LocalePin : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        generator = GameObject.Find("PlanetSystem").GetComponent<Generator>();
    ...
    void LateUpdate()
    {
        ...
        // label.text = string.Format("{9}\n{0:d2}:{6:d2}\n{1:n2}°{2}, {3:n2}°{4}\n{7}\n{8}\n{5}", localTime.hour, Mathf.Abs(linkedLocale.longLatCoord.y), linkedLocale.longLatCoord.y > 0 ? "N" : "S", Mathf.Abs(linkedLocale.longLatCoord.x), linkedLocale.longLatCoord.x > 0 ? "E" : "W", generator.useMiles ? linkedLocale.avgElevation * 3.28084f + "'" : linkedLocale.avgElevation + " m", localTime.minute, linkedLocale.xyzCoord.ToString(), linkedLocale.uvqCoord.ToString(), linkedLocale.placeName);
    }
}
